...
- Download the data -> Split into Training and Test sets -> Data Preprocessing -> Model Training -> Model Evaluation

## II. Pipline Components 

- Download-Task |--\[dataset\]--> Split-Task |--\[train-set/test-set\]--> Preprocessing Task |--->(1)

- (1)--\[preprocessed-train-set\]--> Training Task
- (1)--\[preprocessed-test-set\]--> Evaluation Task

## III. Working

### 1. Install kfp in python workspace 

```bash
!pip install --upgrade kfp==2.14.6
```
...
from kfp.v2 import dsl # creating pipeline

from kfp.v2.dsp import component # building block

# type annotation for the component artifacts
...
def download_data(url: str, output_csv:Output[Dataset]):
    import pandas as pd 
    df = pd.read_excel(url) # shuffle
    df = df.sample(frac=1).rest_index(drop=True) # remove old index
    df.to_csv(output_csv.path, index=False)

```

- artifact's metadata saved in "**output_component_file**" params (in ex is "download_data_component.yaml")

- how to load output of the component?

```python
web_download_op = kfp.components.load_component_from_url("download_data_component.yaml")
```

- another  components for split data, has Input/Ouput annotation
...
- run all component into the pipeline below
```python
@dsl.pipeline(
    name="my-pipeline",
)
...
- generate the pipeline specification file

```python
kfp.compiler.Compiler(mode=kfp.dsl.PipelineExecutionModel.V2_COMPATIBLE).compile(
    piepline_func=my_pipeline,
    package_path="pipeline.yaml"
)
```

- After compile pipeline, you should upload artifact to Pipelines by *Piepeline-UI*
...
